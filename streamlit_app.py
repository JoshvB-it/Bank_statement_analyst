...
import altair as alt  # optional (keep if you add charts later)

# ---------------- UI / Meta ----------------
st.set_page_config(page_title="FNB Statement Analyzer", layout="wide")

PARSER_VERSION = "2025-08-08-r2"
...
# Accept ?pw=... too (works on Streamlit Cloud and local)
try:
    qparams = getattr(st, "query_params", None)
    if callable(qparams):
        qparams = qparams()
    if qparams is None:
...
if isinstance(qparams, dict):
    # Streamlit may return dict[str, list[str]]
    if "pw" in qparams:
        qpw = qparams["pw"][0] if isinstance(qparams["pw"], list) else qparams["pw"]

if "pw_ok" not in st.session_state:
    st.session_state.pw_ok = (qpw == DEFAULT_PASSWORD)

if not st.session_state.pw_ok:
    st.info("Enter the password to proceed.")
...
def clean_amount(raw: str) -> float:
    """
    Accepts:
      '1,234.56', 'R 1,234.56', '(1,234.56)', '-1,234.56', '1,234.56 CR/DR', 'CR 1,234.56'
...
AMOUNT_TAIL_RE = re.compile(
    r"""^
    (?:
        (?:[Rr]\s*)?(\(?-?\d{1,3}(?:,\d{3})*\.\d{2}\)?) (?:\s*(?:CR|DR))?
      | (?:CR|DR)\s*(\(?-?\d{1,3}(?:,\d{3})*\.\d{2}\)?)
    )
    $""",
    re.VERBOSE,
)
...
def main():
    ...
    if not df_bal.empty:
        ...
        if any_diff.any():
            st.warning("Some statements have differences. Expand Diagnostics above and check **Unparsed candidate lines**.")
        else:
